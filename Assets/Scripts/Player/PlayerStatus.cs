...
public class PlayerStatus : MonoBehaviour
{
    ...
    private SystemQuebrar SQ;
    public void Start()
    {
        SQ = FindObjectOfType<SystemQuebrar>();
    }
    ...
    public void raycast(RaycastHit hitInfo)
    {
        ...
        Debug.Log(obj);
        if (hitInfo.collider.gameObject.GetComponent<SystemQuebrar>() != null)
        {
            if (hitInfo.collider.gameObject.tag.Equals("Tree"))
            {
                SQ.Quebrar(hitInfo.collider.gameObject, Opcoes.Tree, hitInfo);
            }
            else if (hitInfo.collider.gameObject.tag.Equals("Rocha"))
            {
                SQ.Quebrar(hitInfo.collider.gameObject, Opcoes.rock, hitInfo);
            }
        }
